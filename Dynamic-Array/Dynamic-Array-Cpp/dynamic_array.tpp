 ...
 */
template <typename E>
MyVec<E>::MyVec() : m_size{0}, m_capacity{1}, mp_data(new E[1])
{
}
 ...
 */
template <typename E>
MyVec<E>::MyVec(size_t in_capacity) : m_size(0), m_capacity(in_capacity), mp_data(new E[in_capacity])
{
}
 ...
 */
template <typename E>
MyVec<E>::MyVec(MyVec &other) : m_size{other.m_size}, m_capacity{other.m_capacity}
{
...
MyVec<E>::MyVec(MyVec &&other) noexcept
    : m_size{std::move(other.m_size)}, m_capacity{std::move(other.m_capacity)}, mp_data{std::move(other.mp_data)}
{
    // for debug
...
template <typename E>
MyVec<E> &MyVec<E>::operator=(MyVec &&other)
{
    if (this != &other)
    {
        delete[] mp_data;        // Free existing memory
...
E &MyVec<E>::emplace_back(Args &&...args)
{
    // Check availability
 ...
 */
template <typename E>
bool MyVec<E>::push_back(const E &data)
{
